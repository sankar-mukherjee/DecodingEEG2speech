...
from sklearn.linear_model import Ridge as L2_LReg
...
def Neighbours_idx(ch_name, channels):
    neighboursDict= np.load('neighbours.npy').item()
    neighbours= neighboursDict[ch_name]
    neighbours_indexes = [channels.index(ch) for ch in neighbours]
...
def Band_filtering(X_orig, Features_orig, band):
    ...
    if band=='original':
        ...
        X_freq = X_orig.filter(1, 12, method='iir', iir_params=filt_params)
        Y_envelope_sp_freq = Features_orig.filter(1, 12, method='iir', iir_params=filt_params).get_data()[:, 0, :]
        Y_lips_ap_freq = Features_orig.filter(1, 12, method='iir', iir_params=filt_params).get_data()[:, 2, :]
...
def Multitaper(X_orig, Features_orig, band):
    ...
    if band=='original':
        ...
        freqs = [1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5,5, 5.5, 6, 6.5, 7, 7.5, 8, 8.5,9, 9.5, 10, 10.5, 11, 11.5, 12]
        X_freq= np.mean(TF(X_orig, freqs, 3, time_bandwidth=4.0, use_fft=True, return_itc=False, decim=1, average=False)._data,2)
        Features_freq = TF(Features_orig, freqs, 3, time_bandwidth=4.0, use_fft=True, return_itc=False, decim=1, average=False)
        Y_envelope_sp_freq = np.mean(Features_freq._data[:, 0, :, :], 1)
        Y_lips_ap_freq = np.mean(Features_freq._data[:, 2, :, :], 1)
...
def decoding_withKfold(X,Y_speech,Y_lips,n_fold,train_index,test_index,examples,feature,regularization=0,shuffle=False): #if regularization is zero this function is a linear regression
    ...
    for k in range(0, n_fold):
        ...
        speechModel = L2_LReg(alpha=regularization)
        lipsModel = L2_LReg(alpha=regularization)
        ...
        if shuffle == True:
            p1_TRAIN = np.random.permutation(X_TRAIN.shape[0])
            p2_TRAIN = np.random.permutation(X_TRAIN.shape[0])
            # p1_TEST = np.random.permutation(X_TEST.shape[0])
            # p2_TEST = np.random.permutation(X_TEST.shape[0])

            X_TRAIN = X_TRAIN[p1_TRAIN,:]
            X_TRAIN = X_TRAIN[p2_TRAIN, :]
            # X_TEST = X_TEST[p1_TEST,:]
            # X_TEST = X_TEST[p2_TEST, :]
        ...
        if examples=='are_Time':   #dividing recostruction in trials (each prediction here is just one time point)
...
def decoding(band,frequencyExtraction,regularization,n_fold,subject_name, savepath,examples='are_Trials',feature='linear',useNeighbours=False,shuffle=False):
    ...
    for N, s in enumerate(subject_name):
        ...
        # elif 'FC5+FC6' in savepath:
        #     pick = ['FC5','FC6']
        #     X_orig = mne.read_epochs(data_path + s + eeg).pick_channels(pick)
        # elif 'FC5/' in savepath:
        #     pick = ['FC5']
        #     X_orig = mne.read_epochs(data_path + s + eeg).pick_channels(pick)
        # elif 'FC6/' in savepath:
        #     pick = ['FC6']
        #     X_orig = mne.read_epochs(data_path + s + eeg).pick_channels(pick)
        ...
        for c, ch_name in enumerate(channels):
            if useNeighbours==False :   X_orig_oneCh=X_orig[:,c:c+1,:]
            else: X_orig_oneCh=X_orig[:,Neighbours_idx(ch_name,channels),:]

            for i, r in enumerate(regularization):

               print('sub '+s+' -channel #'+str(c)+' -features:'+feature+' - reg parameter #'+str(i))
               ...
               for j, t_start in enumerate(T): ##estracting the temporal interval of interest
                    ...
                    [speech_score, lips_score, predictions_speechh, predictions_lipss, speech, lips]=decoding_withKfold(X,Y_envelope_sp,Y_lips_ap,n_fold,train_index,test_index,examples,feature,r,shuffle)
        ...
        if useNeighbours == True: ng = '_yesNeighbours'
        else: ng = '_noNeighbours'

        if shuffle == True: sh = '_yesShuffled'
        else: sh = '_noShuffled'


        np.save(savepath + '/score_speech_all_sub_' + examples + '_' + frequencyExtraction + '_' + feature + ng + sh + '_'+s, score_speech_oneSub)
        np.save(savepath + '/score_lips_all_sub_' + examples + '_' + frequencyExtraction + '_' + feature + ng + sh + '_'+s, score_lips_oneSub)
        np.save( savepath + '/predictions_speech_all_sub_' + examples + '_' + frequencyExtraction + '_' + feature + ng + sh + '_'+s, predictions_speech_oneSub)
        np.save( savepath + '/predictions_lips_all_sub_' + examples + '_' + frequencyExtraction + '_' + feature + ng + sh + '_'+s, predictions_lips_oneSub)
        np.save(savepath + '/true_speech_all_sub_' + examples + '_' + frequencyExtraction + '_' + feature + ng + sh + '_'+s, true_speech_oneSub)
        np.save(savepath + '/true_lips_all_sub_' + examples + '_' + frequencyExtraction + '_' + feature + ng + sh + '_'+s, true_lips_oneSub)
        ...
        if N ==0:
            tmp_results_speech= np.expand_dims(score_speech_oneSub,axis=-1)
            tmp_results_lips= np.expand_dims(score_lips_oneSub,axis=-1)
        else:
            tmp_results_speech = np.concatenate((tmp_results_speech, np.expand_dims(score_speech_oneSub,axis=-1)), -1)
            tmp_results_lips = np.concatenate((tmp_results_lips, np.expand_dims(score_lips_oneSub,axis=-1)), -1)
    ...
    if N !=0:
        ...
        np.save(savepath+'/GAVG_sp_'+examples+'_'+frequencyExtraction+'_'+feature+ng+sh,GAVG_sp)
        np.save(savepath+'/GAVG_lip_'+examples+'_'+frequencyExtraction+'_'+feature+ng+sh,GAVG_lip)
        np.save(savepath+'/GAVG_sp_std_'+examples+'_'+frequencyExtraction+'_'+feature+ng+sh,GAVG_sp_std)
        np.save(savepath+'/GAVG_lip_std_'+examples+'_'+frequencyExtraction+'_'+feature+ng+sh,GAVG_lip_std)
...
if __name__ == '__main__':
    ...
    s='./results_RidgeReg/single_channels/'
    s=s+fband

    # feature = 'derivative1'
    # decoding(band=fband, frequencyExtraction=fmethod,regularization=r, n_fold=5, subject_name=sub, savepath=sbest,examples=regmethod,feature=feature)
    # feature='derivative2'
    # decoding(band=fband, frequencyExtraction=fmethod,regularization=r, n_fold=5, subject_name=sub, savepath=sbest,examples=regmethod,feature=feature)
    # feature = 'pca'
    # decoding(band=fband, frequencyExtraction=fmethod, regularization=r, n_fold=5, subject_name=sub, savepath=sbest,examples=regmethod, feature=feature)
    ...
    feature = 'linear'
    # decoding(band=fband, frequencyExtraction=fmethod, regularization=r, n_fold=2, subject_name=sub, savepath=s,
    #          examples=regmethod, feature=feature,shuffle=False,useNeighbours=False)
    decoding(band=fband, frequencyExtraction=fmethod, regularization=r, n_fold=5, subject_name=sub, savepath=s,
             examples=regmethod, feature=feature, shuffle=False, useNeighbours=True)
    decoding(band=fband, frequencyExtraction=fmethod, regularization=r, n_fold=5, subject_name=sub, savepath=s,
             examples=regmethod, feature=feature, shuffle=True, useNeighbours=True)
