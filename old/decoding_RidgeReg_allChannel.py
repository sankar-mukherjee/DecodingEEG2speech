...
def decoding_withKfold(X,Y_speech,Y_lips,n_fold,train_index,test_index,examples,feature,regularization=0): #if regularization is zero this function is a linear regression
...
def decoding(band,frequencyExtraction,regularization,n_fold,subject_name, savepath,examples='are_Trials',feature='linear'):
    ...
    for s in subject_name:
        ...
        score_speech_oneSub=np.zeros((len(T),len(regularization)),dtype="float32")  # dimensions:(n_channels, n_delays, n_reg_params)
        ...
        for i, r in enumerate(regularization):
           ...
           for j, t_start in enumerate(T): ##estracting the temporal interval of interest
                ...
                [speech_score, lips_score, predictions_speechh, predictions_lipss, speech, lips]=decoding_withKfold(X,Y_envelope_sp,Y_lips_ap,n_fold,train_index,test_index,examples,feature,r)
...
if __name__ == '__main__':
    ...
    s='./results_RidgeReg/all_channels/'
    sbest=s+fband
    ...
    # decoding(band=fband, frequencyExtraction=fmethod, regularization=r, n_fold=5, subject_name=sub, savepath=sbest,examples=regmethod, feature=feature)
    feature = 'linear'
    decoding(band=fband, frequencyExtraction=fmethod, regularization=r, n_fold=5, subject_name=sub, savepath=sbest,
             examples=regmethod, feature=feature)

    s = './results_RidgeReg/best_channels/'
    sbest = s + fband
    ...
    s = './results_RidgeReg/FC5/'
    sbest = s + fband

    # feature = 'derivative1'
    # decoding(band=fband, frequencyExtraction=fmethod, regularization=r, n_fold=3, subject_name=sub, savepath=sbest,
    #          examples=regmethod, feature=feature)
    # feature = 'derivative2'
    # decoding(band=fband, frequencyExtraction=fmethod, regularization=r, n_fold=3, subject_name=sub, savepath=sbest,
    #          examples=regmethod, feature=feature)
...
# fmethod = 'band_pass'    # decoding(band=fband, frequencyExtraction=fmethod, regularization=r, n_fold=3, subject_name=sub, savepath=sbest,
