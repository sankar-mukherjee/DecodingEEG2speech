...
def epoch(raw, mat, Tmin, Tmax):
    ...
    for i in range(0, len(epochs_resampled)):
        a = np.vstack((envelop[IDX[i]][0], jawaopening[IDX[i]][0].T, lipaparature[IDX[i]][0].T,
                       lipProtrusion[IDX[i]][0].T, TTCD[IDX[i]][0].T, TMCD[IDX[i]][0].T, TBCD[IDX[i]][0].T))
...
def coherence_measure(epochs, fmin, fmax, sfreq, indices):
    ...
    con, freqs, times, n_epochs, n_tapers = spectral_connectivity(epochs, method='coh', mode='multitaper', sfreq=sfreq,
                                                                  ...
                                                                  tmin=tmin, mt_adaptive=False, block_size=1000,
...
def coherence_preprocess_delay(epochs, remove_first, d, trial_len, extra_channels, eeg_channles, info, ch_names,
                               event_id, iter_freqs, indices, condition):
    eeg = epochs.copy().drop_channels(extra_channels)
    speech = epochs.copy().drop_channels(eeg_channles)
    speech.drop_channels(ch_names)
...
def coherence_preprocess_delay_surrogate(epochs, remove_first, d, trial_len, features, eeg_channles, info, ch_names,
                                         event_id, no_surrogates, iter_freqs, indices):
